  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if (!$.isLogin) {
        ...
        if ($.isNode()) {
          await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\ncookie已失效，请访问http://jd.shengtao.xyz/updateCookie自主更新cookie`, { index: $.index });
        }
        continue
      }
      await bean();
...
async function showMsg() {
  ...
  if ($.isNode()) {
    await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName}`, `账号${$.index}：${$.nickName || $.UserName}当月收入(截至昨日)：${$.allincomeBean}京豆 🐶\n当月支出(截至昨日)：${$.allexpenseBean}\n昨日收入：${$.incomeBean}京豆 🐶\n昨日支出：${$.expenseBean}京豆 🐶\n当前京豆：${$.beanCount}京豆 🐶${$.message}`, { url: `https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean`, index: $.index })
  }
