  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if (process.env.DREAMFACTORY_FORBID_ACCOUNT) {
        let forbidAccount = process.env.DREAMFACTORY_FORBID_ACCOUNT
        if (forbidAccount.indexOf((i + 1)) > -1) {
          continue;
        }
      }
      cookie = cookiesArr[i];
      ...
      if (!$.isLogin) {
        ...
        if ($.isNode()) {
          await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\ncookie已失效，请访问http://jd.shengtao.xyz/updateCookie更新cookie`, { index: $.index });
        }
        continue
      }
            ...
            if (data['ret'] === 0) {
              ...
              if (data.factoryList && data.productionList) {
                ...
                if (production.investedElectric >= production.needElectric) {
                  ...
                  if (production['exchangeStatus'] === 3) {
                    ...
                    if (new Date().getHours() === 9) {
                      ...
                      if ($.isNode()) await notify.sendNotify(`【京东账号${$.index}】-${$.nickName}`, `京东账号${$.index}[${$.nickName}]【生产商品】${$.productName}兑换已超时，请选择新商品进行制造`, { index: $.index });
                    }
                  }
                  // await exchangeProNotify()
                } else {
                ...
                if (production.status === 3) {
                  ...
                  if ($.isNode()) await notify.sendNotify(`【京东账号${$.index}】- ${$.nickName}`, `京东账号${$.index}[${$.nickName}]【生产商品】${$.productName}\n【超时未完成】已失效，请选择新商品进行制造`, { index: $.index });
                }
              } else {
                ...
                if (!data.factoryList) {
                  ...
                  if ($.isNode()) await notify.sendNotify(`${$.name} - 京东账号${$.index} - ${$.nickName}`, `京东账号${$.index}[${$.nickName}]京喜工厂活动未开始\n请手动去京东APP->游戏与互动->查看更多->京喜工厂 开启活动`, { index: $.index });
